...
See the [test.yml](https://github.com/speria-jp/terraform-provider-googleworkspace/blob/main/.github/workflows/test.yml) file for which version of Go to use while developing the provider. You can manage it automatically using [`goenv`](https://github.com/syndbg/goenv).
...
Configuring tests is similar to configuring the provider; see the [Provider Configuration Reference](https://registry.terraform.io/providers/speria-jp/googleworkspace/latest/docs) for more details. The following environment variables must be set in order to run tests:
...
2. Iterate through the list and determine if a resource is [sweepable](https://github.com/speria-jp/terraform-provider-googleworkspace/blob/main/internal/provider/googleworkspace_sweeper_test.go#L19)
...
Setup:
```bash
mkdir -p ~/.terraform.d/plugins/registry.terraform.io/speria-jp/googleworkspace/5.0.0/darwin_amd64
ln -s $GOBIN/terraform-provider-googleworkspace ~/.terraform.d/plugins/registry.terraform.io/speria-jp/googleworkspace/5.0.0/darwin_amd64/terraform-provider-googleworkspace_v5.0.0
```
...
provider_installation {
	filesystem_mirror {
		...
		include = ["registry.terraform.io/speria-jp/googleworkspace"]
	}
}
```
...
```bash
rm -rf ~/.terraform.d/plugins/registry.terraform.io/speria-jp/googleworkspace/
```
